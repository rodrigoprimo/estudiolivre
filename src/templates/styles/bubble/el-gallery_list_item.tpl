<div id="gListItem">
  ...
  <div id="gLicenseThumbContainer">
    <div id="gLicense">
     {tooltip name="list-descricao-licenca" text=$arquivo.descricaoLicenca}<a href="{$arquivo.linkHumanReadable}"><img src="styles/{$style|replace:".css":""}/img/{$arquivo.linkImagem}"></a>{/tooltip}
    </div>
    <div id="gThumb">
	  ...
	  {else}
		  <img height="51" src="styles/{$style|replace:".css":""}/img/iThumb{$arquivo.tipo}.png">
      {/if}
      </a>
    </div>
  </div>
	...
	{tr}por{/tr}: <a href="el-user.php?view_user={$arquivo.user}">{$arquivo.user}</a> {tr}em{/tr}: {$arquivo.data_publicacao|date_format:"%d/%m/%Y"}
      ...
      <div id="gDownloadRating">
        ...
        <div id="gRatingComments">
          <div id="gRating">
	    <span onmouseout="nd();" onmouseover="tooltip('{$arquivo.ratings} {tr}votos{/tr}<br>{tr}Avaliação - entre na página do arquivo para votar{/tr}')"><img alt="{$arquivo.rating} estrelas" src="styles/{$style|replace:".css":""}/img/star{math equation="round(x)" x=$arquivo.rating|default:"blk"}.png"></span>
	  </div>
	  <div id="gComments">
