{css}
{* $Header: /home/rodrigo/devel/arca/estudiolivre/src/templates/styles/bubble/Attic/tiki-show_page.tpl,v 1.2 2006-10-12 00:29:24 rhwinter Exp $ *}
	...
	{ if $print_page ne 'y'}
		{if !$lock}
			{if $tiki_p_edit eq 'y' or $page|lower eq 'sandbox'}
				{if $beingEdited eq 'y'}
					{tooltip name="show-page-editar-pagina" text="<b>Editar</b> essa página"}<span class="tabbut"><a {*title="{$semUser}"*} class="highlight" href="tiki-editpage.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiEdit.png"></a></span>{/tooltip}
				{else}
					{tooltip name="show-page-editar-pagina" text="<b>Editar</b> essa página"}<span class="tabbut"><a href="tiki-editpage.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiEdit.png"></a></span>{/tooltip}
				{/if}
			{/if}
		{/if}
		...
		{if $feature_wiki_pdf eq 'y'}
			{tooltip name="show-page-criar-pdf" text="Criar um <b>PDF</b> dessa página"}<a {*title="{tr}create pdf{/tr}"*} href="tiki-config_pdf.php?{if $home_info && $home_info.page_ref_id}page_ref_id={$home_info.page_ref_id}{else}page={$page|escape:"url"}{/if}"><img src="styles/{$style|replace:".css":""}/img/iWikiExport.png" alt="{tr}pdf{/tr}"></a>{/tooltip}
		{/if}
		
		{if $page|lower ne 'sandbox'}
			{if $tiki_p_remove eq 'y'}
				{tooltip name="show-page-remover-pagina" text="<b>Remover</b> essa página"}<span class="tabbut"><a href="tiki-removepage.php?page={$page|escape:"url"}&amp;version=last" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiRemove.png"></a></span>{/tooltip}
			{/if}
			{if $tiki_p_rename eq 'y'}
				{tooltip name="show-page-renomear-pagina" text="<b>Renomear</b> página"}<span class="tabbut"><a href="tiki-rename_page.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiRename.png"></a></span>{/tooltip}
			{/if}
		{/if}
		...
		{if $page|lower ne 'sandbox'}
			{if $lock and ($tiki_p_admin_wiki eq 'y' or ($user and ($user eq $page_user or $user eq "admin") and ($tiki_p_lock eq 'y') and ($feature_wiki_usrlock eq 'y')))}
				{tooltip name="show-page-observar-pagina" text="<b>Destravar</b> a página"}<span class="tabbut"><a href="tiki-index.php?page={$page|escape:"url"}&amp;action=unlock" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiUnLock.png"></a></span>{/tooltip}
			{/if}
			{if !$lock and ($tiki_p_admin_wiki eq 'y' or (($tiki_p_lock eq 'y') and ($feature_wiki_usrlock eq 'y')))}
				{tooltip name="show-page-travar-pagina" text="<b>Travar</b> essa página"}<span class="tabbut"><a href="tiki-index.php?page={$page|escape:"url"}&amp;action=lock" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiLock.png"></a></span>{/tooltip}
			{/if}
			{if $tiki_p_admin_wiki eq 'y'}
				{tooltip name="show-page-modificar-permissoes" text="Modificar <b>permissões</b>"}<span class="tabbut"><a href="tiki-pagepermissions.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiPermissions.png"></a></span>{/tooltip}
			{/if}
		{/if}
		
		{if $page|lower ne 'sandbox'}
			{if $feature_history eq 'y' and $tiki_p_wiki_view_history eq 'y'}
				{tooltip name="show-page-historico-edicoes" text="<b>Histórico</b> de edições da página"}<span class="tabbut"><a href="tiki-pagehistory.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$style|replace:".css":""}/img/iWikiHistory.png"></a></span>{/tooltip}
			{/if}
		{/if}
		...
		{if $feature_multilingual == 'y' and count($trads) > 1}
			{tooltip text="<b>Traduções</b> dessa página"}<img src="styles/{$style|replace:".css":""}/img/iWikiTranslations.png" onclick="document.getElementById('transHide').style.display='inline'" style="cursor:pointer">{/tooltip}
		...
		{if $feature_backlinks eq 'y' and $backlinks}
			{tooltip name="show-page-ver-referencias" text="Ver as páginas que referenciam esta página. Ou seja, que possuem links que trazem cá"}<img src="styles/{$style|replace:".css":""}/img/iWikiRef.png" onclick="flip('backlinksId')" style="cursor:pointer">{/tooltip}
		{/if}
