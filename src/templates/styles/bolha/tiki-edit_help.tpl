{* $Header: /cvsroot/arca/estudiolivre/src/templates/styles/bolha/tiki-edit_help.tpl,v 1.1 2006-10-20 21:11:33 rhwinter Exp $ *}
...
<p>{tr}For more information, please see <a href="{$prefs.helpurl}WikiSyntax">WikiSyntax</a>{/tr}</p>
...
<tr><td><strong>{tr}Insert Module Output{/tr}:</strong></td><td> {tr}{literal}{MODULE(module=>some_module)}text{MODULE}{/literal}  can be used to insert the output of module "some_module" into your Wiki page. See <a href="{$prefs.helpurl}PluginModule">PluginModule</a> for more information. {/tr}</td></tr>
<tr><td><strong>{tr}Rendering Program Code{/tr}:</strong></td><td> {tr}{literal}{CODE()}some code{CODE} {/literal} will render "some code" as program code. This plugin has other options; see <a href="{$prefs.helpurl}PluginCode">PluginCode</a>.{/tr}</td></tr>
