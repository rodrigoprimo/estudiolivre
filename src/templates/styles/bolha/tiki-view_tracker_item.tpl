<h1>
	<a class="pagetitle" href="tiki-view_tracker_item.php?trackerId={$trackerId}&amp;itemId={$itemId}">
		{tr}Tracker item:{/tr} {$tracker_info.name}
	</a>
</h1>
<div>
	<span class="button2">
		<a href="tiki-list_trackers.php" class="linkbut">
			{tr}List trackers{/tr}
		</a>
	</span>
	<span class="button2">
		<a href="tiki-view_tracker.php?trackerId={$trackerId}" class="linkbut">
			{tr}View this tracker items{/tr}
		</a>
	</span>
	{if $user}
		<span class="button2">
			<a href="tiki-view_tracker_item.php?itemId={$itemId}&amp;trackerId={$trackerId}&amp;monitor=1" class="linkbut">
				{tr}{$email_mon}{/tr}
			</a>
		</span>
	{/if}
	{if $tiki_p_admin_trackers eq 'y'}
		&nbsp;&nbsp;
		<span class="button2">
			<a href="tiki-admin_trackers.php" class="linkbut">
				{tr}Admin trackers{/tr}
			</a>
		</span>
		<span class="button2">
			<a href="tiki-admin_trackers.php?trackerId={$trackerId}" class="linkbut">
				{tr}Edit this tracker{/tr}
			</a>
		</span>
		<span class="button2">
			<a href="tiki-admin_tracker_fields.php?trackerId={$trackerId}" class="linkbut">
				{tr}Edit fields{/tr}
			</a>
		</span>
	{/if}
...
<div>
	...
	<span class="button2">
		<a href="tiki-view_tracker.php?trackerId={$trackerId}&amp;offset={$offset}&amp;sort_mode={$sort_mode}{foreach key=urlkey item=urlval from=$urlquery}&amp;{$urlkey}={$urlval|escape:"url"}{/foreach}">
			{tr}back{/tr} {tr}items list{/tr}
		</a>
	</span>
	<span class="button2">
		{if $prevmsg}
			...
			{elseif $cur_field.type ne 'x'}
				...
				{else}
					...
					{if ($cur_field.type eq 'l' and $cur_field.options_array[4] eq '1')}
						<br />
						<a href="tiki-view_tracker.php?trackerId={$cur_field.options_array[0]}&amp;filterfield={$cur_field.options_array[1]}&amp;filtervalue={section name=ox loop=$ins_fields}{if $ins_fields[ox].fieldId eq $cur_field.options_array[2]}{$ins_fields[ox].value}{/if}{/section}">
							{tr}Filter Tracker Items{/tr}
						</a>
						<br />
					{/if}
				{/if}
				{if $cur_field.type eq 'f' or $cur_field.type eq 'j'}
					{$cur_field.value|tiki_long_date}		
				...
				{elseif $cur_field.type eq 'u'}
					<a href="tiki-user_information.php?view_user={$cur_field.value|escape:"url"}">
						{$cur_field.value}
					</a>		
				...
				{elseif $cur_field.type eq 'e'}
					...
					{cycle name="1_$fca" values="," advance=false print=false}
				...
				{elseif $cur_field.type eq 'y'}
					...
					{if $o_opt ne '1'}
						<img border="0" src="img/flags/{$cur_field.value}.gif" />
					{/if}
					{if $o_opt ne '1' and $o_opt ne '2'}
						&nbsp;
					{/if}
					{if $o_opt ne '2'}
				...
				{elseif $cur_field.type eq 't' or $cur_field.type eq 'n'}
					{if $cur_field.options_array[2]}
						<span class="formunit">{$cur_field.options_array[2]|escape}&nbsp;</span>
					{/if}
					{$cur_field.value|escape:"html"|default:"&nbsp;"}
					{if $cur_field.options_array[3]}
						<span class="formunit">&nbsp;{$cur_field.options_array[3]|escape}</span>
					{/if}		
					{if $cur_field.options_array[0] eq '1' and $stick ne 'y'}
						{assign var=stick value="y"}
					{else}
				...
				{elseif $cur_field.type eq 'r'}
					{if $cur_field.options_array[2] eq '1'}
						<a href="tiki-view_tracker_item.php?trackerId={$cur_field.options_array[0]}&amp;itemId={$cur_field.linkId}" class="link">
...
{if $tracker_info.useComments eq 'y'}
	...
	{tr}Title{/tr}:<input type="text" name="comment_title" value="{$comment_title|escape}"/>
	{tr}Comment{/tr}:<textarea rows="4" cols="50" name="comment_data">{$comment_data|escape}</textarea>
	...
	<div id="content{cycle name=content assign=focustab}{$focustab}" class="tabcontent"{if $feature_tabs eq 'y'} style="display:{if $focustab eq $cookietab}block{else}none{/if};" {/if}>
		{if $tiki_p_attach_trackers eq 'y'}
			...
			<form enctype="multipart/form-data" action="tiki-view_tracker_item.php" method="post">
				...
				{tr}Upload file{/tr}{if $attach_file}{tr}Edit{/tr}: {/if}<input type="hidden" name="MAX_FILE_SIZE" value="1000000000" /><input name="userfile1" type="file"  />{if $attach_file}<br />{$attach_file|escape}{/if}
				{tr}comment{/tr}<input type="text" name="attach_comment" maxlength="250" value="{$attach_comment|escape}" />
		...
		{section name=ix loop=$atts}
			...
			class="tablename" href="tiki-download_item_attachment.php?attId={$atts[ix].attId}" title="{tr}download{/tr}">
			...
			{if $tiki_p_wiki_admin_attachments eq 'y' or ($user and ($atts[ix].user eq $user))}
				<a class="link" href="tiki-view_tracker_item.php?trackerId={$trackerId}&amp;itemId={$itemId}&amp;removeattach={$atts[ix].attId}&amp;offset={$offset}&amp;sort_mode={$sort_mode}" title="{tr}delete{/tr}">
					<img src="img/icons2/delete.gif" border="0" alt="{tr}delete{/tr}"  hspace="2" vspace="0" />
				</a>
				<a class="link" href="tiki-view_tracker_item.php?trackerId={$trackerId}&amp;itemId={$itemId}&amp;editattach={$atts[ix].attId}&amp;offset={$offset}&amp;sort_mode={$sort_mode}" title="{tr}edit{/tr}">
					<img src="img/icons/edit.gif" border="0" alt="{tr}edit{/tr}"  hspace="2" vspace="0" />
				</a>
			{/if}
		{sectionelse}
	...
	<div id="content{cycle name=content assign=focustab}{$focustab}" class="tabcontent nohighlight"{if $feature_tabs eq 'y'} style="display:{if $focustab eq $cookietab}block{else}none{/if};"{/if}>
		...
		<form enctype="multipart/form-data" action="tiki-view_tracker_item.php" method="post">
			...
			{* --------------------------- to return to tracker list after saving --------- *}
				...
				{if $tiki_p_admin_trackers eq 'y'}
					<a class="link" href="tiki-view_tracker.php?trackerId={$trackerId}&amp;remove={$itemId}" title="{tr}delete{/tr}"><img src="img/icons2/delete.gif" border="0" height="16" width="16" alt='{tr}delete{/tr}' /></a>
				{/if}
			{* ------------------- *}
				...
				{if $cur_field.isHidden eq 'n' or $tiki_p_admin_trackers eq 'y'}		
					...
					{if $cur_field.type ne 'x' and $cur_field.type ne 's'}
						...
						{elseif $cur_field.type eq 'a'}
							...
							rows="{if $cur_field.options_array[2] gt 1}{$cur_field.options_array[2]}{else}4{/if}">
