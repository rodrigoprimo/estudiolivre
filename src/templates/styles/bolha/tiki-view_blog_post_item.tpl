		...
		{if !$printingPost}
			<div class="icons">
				...
				<a href='tiki-send_blog_post.php?postId={$post.postId}'>
					<img src="styles/{$prefs.style|replace:".css":""}/img/iEmail.png" alt="{tr}email this post{/tr}"/>
				</a>
				<a href='tiki-print_blog_post.php?postId={$post.postId}'>
					<img src="styles/{$prefs.style|replace:".css":""}/img/iWikiPrint.png" alt="{tr}print{/tr}" />
				</a>
			</div>				
		{/if}
		{if $use_title eq 'y'}
	...
	<div class="postbody">
		...
		<h4>
			<span>
				tags:
				<em>
					{foreach from=$freetags.data item=tag name=tags}
					{tooltip text="Clique para ver outros posts com a tag <b>"|cat:$tag.tag|cat:"</b>"}<a href="tiki-browse_freetags.php?tag={$tag.tag}&type=blog post">{$tag.tag}</a>{if not $smarty.foreach.tags.last}, {/if}
					{/tooltip}
					{foreachelse}
