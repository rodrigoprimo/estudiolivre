{css}
{* $Header: /cvsroot/arca/estudiolivre/src/templates/styles/bolha/tiki-show_page.tpl,v 1.3 2007-05-10 21:24:05 nano Exp $ *}

<div {if $prefs.user_dbl eq 'y' and $dblclickedit eq 'y' and $tiki_p_edit eq 'y'}ondblclick="location.href='tiki-editpage.php?page={$page|escape:"url"}';"{/if}>	
	...
	{ if $print_page ne 'y'}
		{if !$lock}
			{if $tiki_p_edit eq 'y' or $page|lower eq 'sandbox'}
				{if $beingEdited eq 'y'}
					{tooltip name="show-page-editar-pagina" text="<b>Editar</b> essa página"}<span class="tabbut"><a {*title="{$semUser}"*} class="highlight" href="tiki-editpage.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiEdit.png"></a></span>{/tooltip}
				{else}
					{tooltip name="show-page-editar-pagina" text="<b>Editar</b> essa página"}<span class="tabbut"><a href="tiki-editpage.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiEdit.png"></a></span>{/tooltip}
				{/if}
			{/if}
		{/if}
		
		{if $prefs.wiki_feature_3d eq 'y'}
			{tooltip name="show-page-representacao-3d" text="Representação <b>tridimensional</b> do wiki"}<a {*title="{tr}3d browser{/tr}"*} href="javascript:wiki3d_open('{$page|escape}',{$prefs.wiki_3d_width}, {$prefs.wiki_3d_height})"><img src="styles/{$prefs.style|replace:".css":""}/img/iWiki3dWiki.png"></a>{/tooltip}
		{/if}
		
		{tooltip name="show-page-abrir-impressao" text="Abrir versão para <b>impressão</b>"}<a {*title="{tr}print{/tr}"*} href="tiki-print.php?page={$page|escape:"url"}"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiPrint.png" alt="{tr}print{/tr}" /></a>{/tooltip}
		
		{if $prefs.feature_wiki_pdf eq 'y'}
			{tooltip name="show-page-criar-pdf" text="Criar um <b>PDF</b> dessa página"}<a {*title="{tr}create pdf{/tr}"*} href="tiki-config_pdf.php?{if $home_info && $home_info.page_ref_id}page_ref_id={$home_info.page_ref_id}{else}page={$page|escape:"url"}{/if}"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiExport.png" alt="{tr}pdf{/tr}"></a>{/tooltip}
		{/if}
		
		{if $page|lower ne 'sandbox'}
			{if $tiki_p_remove eq 'y'}
				{tooltip name="show-page-remover-pagina" text="<b>Remover</b> essa página"}<span class="tabbut"><a href="tiki-removepage.php?page={$page|escape:"url"}&amp;version=last" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiRemove.png"></a></span>{/tooltip}
			{/if}
			{if $tiki_p_rename eq 'y'}
				{tooltip name="show-page-renomear-pagina" text="<b>Renomear</b> página"}<span class="tabbut"><a href="tiki-rename_page.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiRename.png"></a></span>{/tooltip}
			{/if}
		{/if}
		...
		{if $page|lower ne 'sandbox'}
			{if $lock and ($tiki_p_admin_wiki eq 'y' or ($user and ($user eq $page_user or $user eq "admin") and ($tiki_p_lock eq 'y') and ($prefs.feature_wiki_usrlock eq 'y')))}
				{tooltip name="show-page-observar-pagina" text="<b>Destravar</b> a página"}<span class="tabbut"><a href="tiki-index.php?page={$page|escape:"url"}&amp;action=unlock" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiUnLock.png"></a></span>{/tooltip}
			{/if}
			{if !$lock and ($tiki_p_admin_wiki eq 'y' or (($tiki_p_lock eq 'y') and ($prefs.feature_wiki_usrlock eq 'y')))}
				{tooltip name="show-page-travar-pagina" text="<b>Travar</b> essa página"}<span class="tabbut"><a href="tiki-index.php?page={$page|escape:"url"}&amp;action=lock" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiLock.png"></a></span>{/tooltip}
			{/if}
			{if $tiki_p_admin_wiki eq 'y'}
				{tooltip name="show-page-modificar-permissoes" text="Modificar <b>permissões</b>"}<span class="tabbut"><a href="tiki-pagepermissions.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiPermissions.png"></a></span>{/tooltip}
			{/if}
		{/if}
		
		{if $page|lower ne 'sandbox'}
			{if $prefs.feature_history eq 'y' and $tiki_p_wiki_view_history eq 'y'}
				{tooltip name="show-page-historico-edicoes" text="<b>Histórico</b> de edições da página"}<span class="tabbut"><a href="tiki-pagehistory.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiHistory.png"></a></span>{/tooltip}
			{/if}
		{/if}
		
		{*if $prefs.feature_likePages eq 'y'}
			{tooltip name="show-page-buscar-similares" text="Buscar páginas <b>similares</b>"}<span class="tabbut"><a href="tiki-likepages.php?page={$page|escape:"url"}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiSimilar.png"></a></span>{/tooltip}
		{/if*}
		
		{if $prefs.feature_multilingual eq 'y' and $tiki_p_edit eq 'y' and !$lock}
		     {tooltip name="show-page-traduzir-pagina" text="<b>Traduzir</b> página"}<span class="tabbut"><a href="tiki-edit_translation.php?page={$page|escape:'url'}" class="tablink"><img src="styles/{$prefs.style|replace:".css":""}/img/iWikiTranslate.png"></a></span>{/tooltip}
		{/if}
		
		{if $prefs.feature_multilingual == 'y' and count($trads) > 1}
			{tooltip text="<b>Traduções</b> dessa página"}<img src="styles/{$prefs.style|replace:".css":""}/img/iWikiTranslations.png" onclick="document.getElementById('transHide').style.display='inline'" style="cursor:pointer">{/tooltip}
		...
		{if $prefs.feature_backlinks eq 'y' and $backlinks}
			{tooltip name="show-page-ver-referencias" text="Ver as páginas que referenciam esta página. Ou seja, que possuem links que trazem cá"}<img src="styles/{$prefs.style|replace:".css":""}/img/iWikiRef.png" onclick="flip('backlinksId')" style="cursor:pointer">{/tooltip}
		{/if}
		
		{if $prefs.feature_freetags eq 'y'}
				{tooltip text="<b>Tags</b> dessa página"}<img src="styles/{$prefs.style|replace:".css":""}/img/iWikiTags.png" onclick="flip('tagsHide')" style="cursor:pointer">{/tooltip}
		...
		{* FEATURE DESABILITADO NO EL: if $user and $prefs.feature_notepad eq 'y' and $tiki_p_notepad eq 'y'}
		...
		{* FEATURE DESABILITADO NO EL: if $user and $prefs.feature_user_watches eq 'y'}
